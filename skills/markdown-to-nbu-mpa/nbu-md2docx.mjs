...
import MarkdownIt from '../markdown-to-word/node_modules/markdown-it/index.mjs';
import {
  Document, Packer, Paragraph, TextRun, Table, TableRow, TableCell, Math as DocxMath,
  MathRun,
  HeadingLevel, AlignmentType, BorderStyle, WidthType, ShadingType,
  LevelFormat, convertInchesToTwip, ExternalHyperlink, ImageRun, VerticalMergeType,
  Tab, TabStopType,
} from '../markdown-to-word/node_modules/docx/dist/index.mjs';
// Image dimensions: parse PNG/JPEG headers directly (no external dep)
import { latexToInlineMath, latexToMathChildren } from '../markdown-to-word/latex2math.mjs';
...
const resourceDirs = [
  inputDir,
  ...(process.env.PANDOC_RESOURCE_PATH || '')
    .split(':')
    .map((item) => item.trim())
    .filter(Boolean)
...
function ensureParagraphBreaks(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (/<table\b/i.test(line)) inHtmlTable = true;
    result.push(lines[i]);
    const skipBreak = inHtmlTable;
    if (/<\/table\s*>/i.test(line)) inHtmlTable = false;
...
function isChineseCaptionText(text, kind) {
  if (!text) return false;
  return new RegExp(`^${kind}\\s*[A-Z0-9一二三四五六七八九十]+[.-]\\d+[ \\u3000]+\\S+`, 'i').test(text.trim());
}
...
function stripLatexTag(latex) {
  return latex.replace(/\\tag\s*\{[^}]+\}/g, '').trim();
}

function latexToReadableText(latex) {
  return stripLatexTag(latex)
    .replace(/\\begin\s*\{\s*cases\s*\}/g, '{ ')
    .replace(/\\end\s*\{\s*cases\s*\}/g, '')
    .replace(/\\\\/g, '; ')
    .replace(/&/g, ' ')
    .replace(/\\text\s*\{([^}]*)\}/g, '$1')
...
function mathChildrenForDisplay(latex) {
  const clean = stripLatexTag(latex);
  if (/\\begin\s*\{\s*cases\s*\}/.test(clean)) {
    return [new MathRun(latexToReadableText(clean))];
  }
  return latexToMathChildren(clean);
}
...
function htmlSpanAttr(attrs, name) {
  const match = attrs.match(new RegExp(`\\b${name}\\s*=\\s*(?:"([^"]+)"|'([^']+)'|([^\\s"'>]+))`, 'i'));
  const value = match ? Number.parseInt(match[1] || match[2] || match[3], 10) : 1;
...
function parseHtmlTableRows(html) {
  ...
  const hasPendingFrom = (start) => pending.some((item, idx) => idx >= start && item && item.start === idx);
  const addPendingCell = (cells, col) => {
    const item = pending[col];
    if (!item || item.start !== col) return 0;
    cells.push({
      text: '',
      header: item.header,
    ...
    item.rowsLeft -= 1;
    for (let k = 0; k < item.colspan; k++) pending[col + k] = null;
    if (item.rowsLeft > 0) {
      for (let k = 0; k < item.colspan; k++) pending[col + k] = item;
    }
    return item.colspan;
  };
  ...
  while ((rowMatch = rowRe.exec(html))) {
    ...
    while ((cellMatch = cellRe.exec(rowHtml))) {
      while (pending[col]) col += addPendingCell(cells, col);
      const tag = cellMatch[1].toLowerCase();
      const attrs = cellMatch[2] || '';
      const colspan = htmlSpanAttr(attrs, 'colspan');
      const rowspan = htmlSpanAttr(attrs, 'rowspan');
      const cell = {
        text: htmlCellText(cellMatch[3] || ''),
        header: tag === 'th',
        colspan,
      ...
      cells.push(cell);
      if (rowspan > 1) {
        const item = { start: col, colspan, rowsLeft: rowspan - 1, header: cell.header };
        for (let k = 0; k < colspan; k++) pending[col + k] = item;
      }
      col += colspan;
    }

    while (hasPendingFrom(col)) {
      if (pending[col]) col += addPendingCell(cells, col);
      else {
        cells.push({ text: '', header: false, colspan: 1 });
        col += 1;
      }
    }
...
function buildDocxTableFromRows(rows, citationState = null) {
  if (!rows || rows.length === 0) return null;
  const BORDER_GRID = { style: BorderStyle.SINGLE, size: 6, color: '000000' };
  const tableRows = rows.map((row, rowIdx) => {
    ...
    const cells = row.map((cell) => {
      ...
      const runs = cell.text
        ? parseInline([{ type: 'text', content: cell.text }], cell.header ? { bold: true } : {}, tableRun, citationState)
        : [new TextRun('')];
      return new TableCell({
        children: [new Paragraph({
          children: wrapLinks(runs),
          spacing: { before: 30, after: 30, line: LINE_SPACING_SINGLE },
          alignment: AlignmentType.LEFT,
        })],
        borders,
        ...(cell.colspan && cell.colspan > 1 ? { columnSpan: cell.colspan } : {}),
...
function createImageRun(tok) {
  ...
  if (!src) return null;
  const candidates = src.startsWith('/') ? [src] : resourceDirs.map((dir) => resolve(dir, src));
  const imgPath = candidates.find((candidate) => existsSync(candidate));
  if (!imgPath) {
    console.warn(`Image not found: ${candidates[0]}`);
    return null;
  }
  try {
    ...
    return new ImageRun({
      type: getImageType(imgData, imgPath),
      data: imgData,
  ...
  while (i < tokens.length) {
    ...
    switch (tok.type) {
      ...
      case 'paragraph_open': {
        ...
        const isFigureCaption = canBeCaption && isChineseCaptionText(plainText, '图');
        const isTableCaption = canBeCaption && isChineseCaptionText(plainText, '表') && tokens[i]?.type === 'table_open';
        ...
        if (equationNumber) {
          elements.push(new Paragraph({
            children: [
              ...
              new DocxMath({ children: mathChildrenForDisplay(tok.content) }),
              new TextRun({ children: [new Tab()] }),
              new TextRun(defaultRun({ text: equationNumber })),
            ],
          ...
          elements.push(new Paragraph({
            ...
            children: [new DocxMath({ children: mathChildrenForDisplay(tok.content) })],
            spacing: { before: 120, after: 120, line: LINE_SPACING },
            indent: { firstLine: 0 },
          }));
        }
      ...
      case 'html_block': {
        if (/<table\b/i.test(tok.content)) {
          const table = buildDocxTableFromRows(parseHtmlTableRows(tok.content), citationState);
          if (table) {
            elements.push(table);
            elements.push(new Paragraph({ spacing: { after: 80 } }));
          }
        } else if (tok.content.trim()) {
          const text = htmlCellText(tok.content);
          if (text) {
            elements.push(new Paragraph({
              children: [new TextRun(defaultRun({ text }))],
              spacing: { before: 0, after: 120, line: LINE_SPACING },
            }));
          }
        }
        i++;
